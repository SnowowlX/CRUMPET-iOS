...
class MiniTailMovesVC: UIViewController{
    ...
    func checkCasualModeIsOn() {
        if AppDelegate_.casualONMinitail || AppDelegate_.walkModeOn || AppDelegate_.moveOn {
            // already started
            [btnSlowWag1, btnSlowWag2, btnSlowWag3, btnFastWag].forEach({
                $0?.isEnabled = false
                $0?.alpha = 0.5
            })
        } else {
    ...
    func setupLocalization()  {
        self.title = NSLocalizedString("kTail Minitail Moves", comment: "")
        ...
        btnFastWag.setTitle(NSLocalizedString("kFastWagMinitail", comment: ""), for: .normal)
    }
